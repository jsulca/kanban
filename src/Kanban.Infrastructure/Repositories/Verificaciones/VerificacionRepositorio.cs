...
{
    public PagedResult<Domain.Genericos.Verificaciones.Verificacion> ListarPorPagina(VerificacionFiltro? filtro, int page, int pageSize)
    {
        var _parametros = new List<NpgsqlParameter>();
        var _queryConditions = new StringBuilder();
        List<Domain.Genericos.Verificaciones.Verificacion> lista = new();
        var totalRows = 0;
        try
        {
            ...
            using (var cmd = CreateCommand())
            {
                ...
                {
                    while (rd.Read())
                        lista.Add(new Domain.Genericos.Verificaciones.Verificacion
                        {
                            Id = rd.GetInt32(0),
            ...
            return new PagedResult<Domain.Genericos.Verificaciones.Verificacion>(lista, totalRows, page, pageSize);
        }
        catch (Exception ex)
        {
    ...
    public List<Domain.Genericos.Verificaciones.Verificacion> Listar()
    {
        List<Domain.Genericos.Verificaciones.Verificacion> lista = new();
        try
        {
    ...
    public Domain.Genericos.Verificaciones.Verificacion? Buscar(int id)
    {
        Domain.Genericos.Verificaciones.Verificacion? entidad = null;
        try
        {
            var _query = new StringBuilder();
            _query.Append(
            ...
            using (var cmd = CreateCommand())
            {
                ...
                {
                    while (rd.Read())
                        entidad = new Domain.Genericos.Verificaciones.Verificacion
                        {
                            Id = rd.GetInt32(0),
                            Nombre = rd.GetString(1),
                            Activo = rd.GetBoolean(2),
    ...
    public bool Guardar(Domain.Genericos.Verificaciones.Verificacion entidad)
    {
    ...
    public void Actualizar(Domain.Genericos.Verificaciones.Verificacion entidad)
    {
        ArgumentNullException.ThrowIfNull(entidad.Nombre);
